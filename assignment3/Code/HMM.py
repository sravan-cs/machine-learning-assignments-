...
def getProbLeftRight(seq,reca,nxta,recaSymb,nxtaSymb):
	...
	return np.sum(alpha)
for symbolCnt,statesCnt in [(10,3),(14,3),(20,3),(20,4)]:
	trainData = []
	devData = []
	for num in digList:
		...
		devData.append(dlst)
	confMat = np.zeros((len(digList),len(digList)))
	tmpdataLst = []
	...
	kmeans = KMeans(n_clusters=symbolCnt,max_iter=50,random_state=134124).fit(dataLst)
	lab = kmeans.labels_
	trainData2 = []
	l = 0
	for c,exampleLst in enumerate(trainData):
	...
	plot_confusion_matrix(predCls,actualCls,len(digList),"CMat" + str(statesCnt) + " " + str(symbolCnt) + ".jpg")
	for lst in confMat:
		for cnt in lst:
			print(cnt,end=' ')
		print()
