...
# svmclf = svm.SVC(C=1.8,probability=True).fit(trainData,trainDataCls)
...
# 	writeFile.write(" ")
# 	probs = svmclf.predict_proba([devData[i]])[0]
# 	for j in range(clsCnt):
# 		writeFile.write(str(probs[j]))
# 		writeFile.write(" ")
# 	writeFile.write("\n")
# writeFile.close()
# print(svmclf.score(devData,devDataCls))
...
# #trainData2,qs = PCA(trainData2,30) #change here for features
# #trainData2,trainDataCls2,qs2 = lda(trainData2,trainDataCls2)
# svmclf2 = svm.SVC(probability=True).fit(trainData2,trainDataCls2)
# devData2 = []
# devDataCls2 = []
# for curCls,imgType in enumerate(imgTypes):
...
# 	writeFile.write(" ")
# 	probs = svmclf2.predict_proba([devData2[i]])[0]
# 	for j in range(clsCnt2):
# 		writeFile.write(str(probs[j]))
# 		writeFile.write(" ")
# 	writeFile.write("\n")
# writeFile.close()
# devDataPred = svmclf2.predict(devData2)
# print(svmclf2.score(devData2,devDataCls2))
...
# svmclf3 = svm.SVC(C=12,probability=True).fit(trainData3,trainDataCls3)
# for i,d in enumerate(devData3):
# 	writeFile.write(str(devDataCls3[i]))
# 	writeFile.write(" ")
# 	probs = svmclf3.predict_proba([devData3[i]])[0]
# 	for j in range(clsCnt3):
# 		writeFile.write(str(probs[j]))
# 		writeFile.write(" ")
# 	writeFile.write("\n")
# writeFile.close()
# devDataPred = svmclf3.predict(devData3)
# print(svmclf3.score(devData3,devDataCls3))
...
writeFile = open("SVM.txt",'w')
clsCnt3 = len(tmp.keys())
svmclf3 = svm.SVC(C=8,probability=True).fit(trainData3,trainDataCls3)
for i,d in enumerate(devData3):
	writeFile.write(str(devDataCls3[i]))
	writeFile.write(" ")
	probs = svmclf3.predict_proba([devData3[i]])[0]
